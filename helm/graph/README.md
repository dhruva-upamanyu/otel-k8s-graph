...
Kubernetes + OTel relationship graph: graph-k8s (K8s API -> Redis structure writer), graph-otel (OTel span metrics -> Redis relationship writer), and graph-read (Redis -> HTTP query API; also the MCP server when run locally).
...
| Component | Role |
|-----------|------|
| **graph-k8s** | Watches the Kubernetes API and writes cluster structure (namespace/node/deployment/pod/container) to Redis. Single authoritative writer. |
...
| graphOtel.config.grpcListenAddr | string | `":4317"` | `GRPC_LISTEN_ADDR`: OTLP/gRPC bind address (collector pushes metrics here). |
